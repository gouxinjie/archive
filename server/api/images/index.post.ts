...
import { parseImageUploadForm } from '../../utils/imagePayload';
...
export default defineEventHandler(async (event) => {
  try {
    ...
    const payload = parseImageUploadForm(formData);
    const id = randomUUID();
    const storagePath = buildModuleFileStoragePath(session.profileId, 'images', id, payload.fileType);
    ...
    try {
      ...
      if (assetCreated) {
        deleteFileAsset(id, session.profileId, 'images');
      }
    ...
    return createSuccessResponse({ id }, '图片上传成功');
  } catch (error: unknown) {
    console.error(getErrorMessage(error));
    return createErrorResponse('IMAGE_CREATE_FAILED', getErrorMessage(error));
  }
});
